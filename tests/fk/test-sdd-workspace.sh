...
main() {
    ...
    if [[ "$dir" == "$repo/.codex-workflow-fk/sdd" ]]; then
        pass "prints <repo-root>/.codex-workflow-fk/sdd"
    else
        fail "prints <repo-root>/.codex-workflow-fk/sdd"
        echo "    got: $dir"
    fi

    if [[ -f "$repo/.codex-workflow-fk/sdd/.gitignore" && "$(cat "$repo/.codex-workflow-fk/sdd/.gitignore")" == "*" ]]; then
        pass "self-ignoring .gitignore created with '*'"
    else
        fail "self-ignoring .gitignore created with '*'"
    fi

    printf 'x\n' > "$repo/.codex-workflow-fk/sdd/artifact.md"
    local status
...
PLAN
    ...
    case "$brief_path" in
        "$repo/.codex-workflow-fk/sdd/"*) pass "task-brief writes its brief under the workspace" ;;
        *)
    ...
    case "$rp_path" in
        "$repo/.codex-workflow-fk/sdd/"*) pass "review-package writes its diff under the workspace" ;;
        *)
    ...
    if [[ "$wt_dir" == "$wt_root/.codex-workflow-fk/sdd" && "$wt_dir" != "$dir" ]]; then
    ...
    printf 'y\n' > "$wt/.codex-workflow-fk/sdd/artifact.md"
    local wt_status
